...
# Streamlit Dashboard - Fixed import structure
def main():
    import streamlit as st
    ...
    with st.sidebar:
        ...
        st.subheader("📊 统计周期")
        show_7d = st.checkbox("7日平均", value=True)
        show_14d = st.checkbox("14日平均", value=True)
        show_21d = st.checkbox("21日平均", value=True)
    ...
    with col1:
        ...
        # Debug info
        st.write(f"📊 总信号数: {len(all_signals)}, 选中代码: {len(selected_codes)}")
        ...
        if filtered_signals:
            ...
            st.error("❌ 没有生成任何交易信号")
            st.write("这可能是因为:")
            st.write("1. 数据文件格式不正确")
            st.write("2. 数据时间范围不足")
            st.write("3. 计算过程中出现错误")
